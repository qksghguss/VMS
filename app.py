...
class MplCanvas(FigureCanvas):
    ...
    def apply_light_style(self):
        ...
        self.ax.spines["left"].set_color("#d8e2f1")
        self.ax.spines["bottom"].set_color("#d8e2f1")
        self.ax.tick_params(colors="#2a3447", labelsize=9)
        self.ax.grid(axis="y", color="#e8eef9", alpha=1.0, linestyle="--", linewidth=0.8)
...
class MainWindow(QMainWindow):
    ...
    def _apply_style(self):
        self.setStyleSheet(
            """
            QWidget { font-size: 13px; color: #1f2a44; }
            QMainWindow, QWidget { background: #f7f9fc; }
            QTabWidget::pane { border: 1px solid #dbe3f3; background: #ffffff; }
            QTabBar::tab {
                min-width: 170px; min-height: 34px; padding: 6px 12px;
                background: #eef3fd; border: 1px solid #d4dff3; color: #2c3f66;
            }
            QTabBar::tab:selected { background: #2f6df6; color: #ffffff; }
            QPushButton {
                min-height: 30px; padding: 3px 10px;
                border: 1px solid #c8d7f4; border-radius: 6px; background: #ffffff; color: #2453c9;
            }
            QPushButton:hover { background: #edf4ff; }
            QLineEdit, QComboBox, QDateEdit, QSpinBox, QListWidget, QTextEdit, QTableWidget {
                min-height: 28px; border: 1px solid #d5e0f2; border-radius: 4px; background: #ffffff; color: #1f2a44;
            }
            QHeaderView::section { background: #f1f6ff; color: #25406f; border: 1px solid #d9e5f7; }
            QLabel#sectionTitle { font-weight: 700; color: #2f6df6; }
            QLabel#miniCard {
                background: #f3f7ff; border: 1px solid #d7e4fb; border-radius: 10px;
                padding: 8px 12px; font-weight: 600; color: #1b3f95;
            }
            """
        )
    ...
    def _build_dashboard_tab(self):
        ...
        self.tbl_sum.setHorizontalHeaderLabels(["순위", "작업자", "작업일보", "매수", "LOT", "L1/L2"])
    ...
    def update_dashboard(self):
        ...
        for r, row in enumerate(df.itertuples(index=False)):
            ...
            self.tbl_sum.setItem(r, 5, QTableWidgetItem(f"{row[5]:,.2f} / {row[6]:,.2f}"))
        ...
        self.canvas_top.apply_light_style()
        top10 = self.summary.head(10)
        if len(top10):
            x = [f"{r['작업자']}\n{r['작업일보']}" for _, r in top10.iterrows()]
            y = list(top10["매수"])
            bars = self.canvas_top.ax.bar(x, y, color="#4a86ff", edgecolor="#2f6df6", linewidth=1.0)
            for b in bars:
                b.set_alpha(0.9)
            self.canvas_top.ax.set_title("TOP10 매수 (작업자 × 작업일보)", color="#234fba", fontsize=12, pad=12)
            self.canvas_top.ax.tick_params(axis="x", rotation=25)
        self.canvas_top.figure.tight_layout()
        self.canvas_top.draw()

        self.canvas_daily.ax.clear()
        self.canvas_daily.apply_light_style()
        ...
        if len(daily_total):
            self.canvas_daily.ax.plot(
                ...
                marker="o", markersize=6, linewidth=2.2, color="#2f6df6"
            )
            self.canvas_daily.ax.fill_between(
                daily_total["기간"], daily_total["매수"],
                [0] * len(daily_total), color="#2f6df6", alpha=0.12
            )
            self.canvas_daily.ax.set_title("기간별 총 매수 추이", color="#234fba", fontsize=12, pad=12)
